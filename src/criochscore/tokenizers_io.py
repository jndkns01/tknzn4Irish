...
Supports four tokenizer families:
...
* ``huggingface`` - a pretrained tokenizer loaded via
  ``transformers.AutoTokenizer.from_pretrained``. ``--tokenizer`` is a
  Hugging Face Hub model id (e.g. ``'DCU-NLP/bert-base-irish-cased-v1'``)
  or a local directory, not a single file. Encoded with ``.tokenize()``.

Any of the four may still use a "special token" marker (e.g. ``'##'`` for
...
ModelType = Literal["sentencepiece", "byte-level", "standard", "huggingface"]


def load_tokenizer(path: str | Path, model_type: ModelType, do_lower_case: bool = False):
    """Load a tokenizer. ``model_type`` must be 'sentencepiece', 'byte-level',
    'standard', or 'huggingface' (see module docstring).

    For 'huggingface', ``path`` is a Hugging Face Hub model id or local
    ...
    if model_type == "huggingface":
        ...
        return AutoTokenizer.from_pretrained(path, do_lower_case=do_lower_case)

    raise ValueError(
        f"Unknown model_type: {model_type!r} "
        "(expected 'sentencepiece', 'byte-level', 'standard', or 'huggingface')"
    )
...
def get_token_lens(
    ...
    for word in words:
        if model_type == "sentencepiece":
            ...
            encoded = tokenizer.tokenize(word)
            token_lens[word] = (len(encoded), list(encoded))
        else:
