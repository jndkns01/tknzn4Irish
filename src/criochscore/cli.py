...
def _prompt_model_type() -> str:
    prompt = (
        ...
        "or another type (WordPiece/BPE/Unigram)? [sentencepiece/byte-level/other]: "
    )
    while True:
        answer = input(prompt).strip().lower()
        if answer in ("sentencepiece", "byte-level"):
            return answer
        if answer in ("other", "standard"):
            return "standard"
        print("Please answer 'sentencepiece', 'byte-level', or 'other'.")
...
def cmd_score(args: argparse.Namespace) -> None:
    ...
    tokenizer = load_tokenizer(args.tokenizer, model_type)
    ...
    tokenizer_name = Path(args.tokenizer).stem
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_score.add_argument("--tokenizer", required=True,
                          help="Path to the tokenizer file (.json for byte-level, .model for sentencepiece).")
    p_score.add_argument("--model-type", choices=["sentencepiece", "byte-level", "standard"], default=None,
                          help="Tokenizer family ('standard' covers WordPiece/BPE/Unigram via the "
                               "tokenizers library). If omitted, you'll be prompted.")
