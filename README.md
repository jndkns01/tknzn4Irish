...
Three `--model-type` values are supported:
...
Only `sentencepiece` and `byte-level` need to be explicitly declared;
anything else defaults to `standard` (the CLI prompt offers it as
`other` if you don't pass `--model-type`).

The special-token question is independent of `--model-type` — a
`standard` WordPiece tokenizer, for example, would still be declared with
`--special-token "##"`.
